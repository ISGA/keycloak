...
* For AWS ALB in passthrough mode use `x-amzn-mtls-clientcert`. +
* For AWS ALB in verify mode use `x-amzn-mtls-clientcert-leaf`.
...
| Enable trusting AWS ALB and NGINX proxy certificate verification, instead of forwarding the certificate to {project_name} and verifying it in {project_name}.
|===
