...
apply the upper bound to. For example, to apply an upper-bound of `1000` to the `actionTokens` cache you would configure
`--cache-embedded-action-tokens-max-count=1000`. An upper bound should not be defined on the `work` cache, therefore we
do not provide a command line option for this cache.
